...
| `dynamic_step_tick`  | `dynamic_step_tick.sv`                      | 仅在 RUN 状态产生障碍下落 tick，并按时间递增下落速度              |
...
| tick         | 默认频率   | 用途            |
|:------------ |:------ |:------------- |
| `step_tick`  | 2Hz -> 3Hz -> 4Hz | RUN 后每 15s 加快一档，最高 4Hz |
...
stateDiagram-v2
    [*] --> IDLE
    IDLE --> RUN: KEY1/2/3/4 选择 20/30/40/50 个障碍 / 启动背景音乐
    RUN --> WIN: 本局障碍全部通过
    RUN --> LOSE: 剩余机会 M = 0
...
- 在 `IDLE` 状态按 `KEY1`/`KEY2`/`KEY3`/`KEY4` 分别选择本局生成 20/30/40/50 批障碍；若多个开始键同时触发，KEY1 优先。
- `dynamic_step_tick` 只在 `ST_RUN` 状态产生 `step_tick`：开局 2Hz，运行 15s 后升到 3Hz，再运行 15s 后升到 4Hz，最高不再继续加速；离开 RUN 后下一局重新从 2Hz 开始。
...
- 当 `M = 0` 时失败；当本局选择的障碍全部生成并完全离开画面后胜利。
...
当前已使用 Verilator 和 Icarus Verilog 13.0 双工具完成检查。Verilator 用于 lint 和自检仿真；Icarus Verilog 使用 `iverilog -g2012` 编译、`vvp` 运行，验证 testbench 在另一个仿真器下也能通过。

已验证场景：

- `dynamic_step_tick` 在 RUN 中按 2Hz、3Hz、4Hz 三档递增，达到 4Hz 后保持最高档，离开 RUN 后复位到 2Hz。
- `KEY1`/`KEY2`/`KEY3`/`KEY4` 开局分别选择 20/30/40/50 个障碍。
- 多个开始键同时触发时 KEY1 优先，选择 20 个障碍。
- 50 障碍模式下 `spawn_count` 能计到 50，不会在 31 后截断。
- 20/30/40/50 四种模式都能沿安全路径无伤进入胜利状态，并启动胜利音乐。
- 胜利后任意键返回初始状态，并输出停止音乐事件。
...
verilator --lint-only -sv \
  ...
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new/dynamic_step_tick.sv \
...
iverilog -g2012 -Wall -s tb_obstacle_car \
  ...
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new/obstacle_car_top.sv \
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sim_1/new/tb_obstacle_car.sv
vvp /private/tmp/tb_obstacle_car_iverilog.vvp
```

```bash
iverilog -g2012 -Wall -s obstacle_car_top \
  -I vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new \
  -o /private/tmp/obstacle_car_top_iverilog.vvp \
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new/tick_gen.sv \
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new/dynamic_step_tick.sv \
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new/hp_led_pwm.sv \
  vivado/Obstacle_Avoidance_Car_Project/Obstacle_Avoidance_Car_Project.srcs/sources_1/new/key_debounce.sv \
...
Icarus Verilog 会提示 `unique case` 的检查属性被忽略，这是仿真器能力提示，不是 RTL 错误，也不影响 Vivado 综合。运行 testbench 时建议显式加入 `-s tb_obstacle_car`，避免 Icarus 把未实例化的顶层也当作 root module 运行，从而触发 `$readmemh` 相对路径问题。
...
1. 打开 `.xpr`，确认 Sources 中能看到 8 个 RTL 文件、`buzzer_pcm_rom.svh` 和 `buzzer_pcm_rom.mem`。
...
6. 下载到开发板后检查：
   ...
   - `KEY1`/`KEY2`/`KEY3`/`KEY4` 开始游戏时分别选择 20/30/40/50 个障碍。
   - `KEY2` 左移、`KEY1` 右移，按键为低有效。
   - 障碍常亮下落，车子底层闪烁；运行中下落速度从 2Hz 温和提升到 3Hz、4Hz。
...
当前版本完成基础功能、KEY1~KEY4 关卡长度选择、动态下落速度、背景/胜败音乐和 PWM 呼吸灯扣血动画。后续若需要继续加分，可以在现有架构上扩展：

- 动态难度：根据剩余生命值、障碍密度或玩家表现进一步调整 `step_tick` 频率。
