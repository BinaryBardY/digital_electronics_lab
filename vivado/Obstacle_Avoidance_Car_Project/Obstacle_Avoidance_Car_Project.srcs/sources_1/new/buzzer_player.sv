...
// 无源蜂鸣器旋律播放器。
//
// 无源蜂鸣器需要输入一定频率的方波才会发声。本模块在 busy 期间根据当前
// 音符频率翻转 beep，形成近似 50% 占空比的 PWM 方波；同时用 note_cnt
// 控制每个音符持续时间。
//
// 控制接口很简单：
// - play_start 拉高一拍：从第 0 个音符开始播放。
// - play_win=1：选择胜利升调旋律；play_win=0：选择失败降调旋律。
// - done 拉高一拍：8 个音符播放完毕。
// -----------------------------------------------------------------------------
module buzzer_player #(
    ...
    // 根据目标频率计算半周期计数值。
    // beep 每经过 half_div 个 clk 翻转一次，因此完整周期约为：
    // 2 * half_div / CLK_HZ 秒。
    ...
    // note_cnt 控制音符时长；tone_cnt 控制当前音符的 beep 翻转频率。
    logic [NOTE_W-1:0] note_cnt;
    logic [TONE_W-1:0] tone_cnt;
    // 当前播放到第几个音符。
    logic [2:0]        note_index;
    // 播放启动时锁存 play_win，避免播放中外部变化影响当前旋律。
    logic              song_win;
            ...
            if (play_start) begin
                ...
                if (note_cnt == NOTE_LAST) begin
                    ...
                    if (note_index == NOTE_INDEX_LAST) begin
                        // 最后一个音符播放完成：结束 busy，发出 done 脉冲并关蜂鸣器。
                        busy       <= 1'b0;
                        done       <= 1'b1;
                        beep       <= 1'b0;
                        note_index <= '0;
