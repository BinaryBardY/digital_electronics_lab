...
// - 按键输入来自 FPGA 外部，对 fpga_clk 来说是异步信号，需要先同步。
// - 游戏核心希望接收“按下一次只触发一次”的事件，而不是持续电平。
//
// 输出含义：
// - key_held[i]    ：防抖稳定后的按住状态，高有效，可持续多拍。
// - key_pressed[i] ：当 key_held 从 0 变为 1 时拉高一个 clk 周期。
//                    松开按键不会产生 key_pressed 脉冲。
// -----------------------------------------------------------------------------
module key_debounce #(
    ...
    input  logic                 rst,
    // 原始按键输入，低电平表示按下。
    input  logic [KEY_COUNT-1:0] key_n,
    // 防抖后的单周期按下脉冲，高有效。
    output logic [KEY_COUNT-1:0] key_pressed,
    // 防抖后的稳定按住状态，高有效。
    output logic [KEY_COUNT-1:0] key_held
);
    // 将防抖时间从 ms 换算为 clk 周期。
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // key_pressed 默认每拍清零，只在“确认按下”的那一拍置 1。
            key_pressed <= '0;
