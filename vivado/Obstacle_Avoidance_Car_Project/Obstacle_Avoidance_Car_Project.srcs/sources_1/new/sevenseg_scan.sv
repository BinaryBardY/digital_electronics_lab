...
// 6 位共阳极数码管动态扫描模块。
//
// 游戏核心输出的是抽象的 obstacle_grid 和 car_col，本模块负责把它们转换成
// 真实数码管位选 dig 和段选 seg：
// - 每次 scan_tick 切换到下一位数码管。
// - 每一列只使用 A/G/D 三个横向段显示游戏三层。
// - 障碍常亮，车子在底层 D 段闪烁。
//
// 硬件极性：
// - dig 低有效：某一位写 0 表示选中该位。
// - seg 低有效：某一段写 0 表示点亮该段。
// - seg 位序为 {DP,G,F,E,D,C,B,A}，内部 seg_on 使用 7 位 {G..A} 的正逻辑。
// -----------------------------------------------------------------------------
module sevenseg_scan (
    ...
    input  logic [17:0] obstacle_grid,
    // 车子所在列，范围 0~5。
    input  logic [2:0]  car_col,
    // 车子是否可见。顶层用 blink_phase 产生闪烁效果。
    input  logic        car_visible,
    // 6 位位选输出，低有效。
    ...
    always_comb begin
        ...
        seg_on[0] = obstacle_grid[col_index * 3 + 0]; // A
        seg_on[6] = obstacle_grid[col_index * 3 + 1]; // G
        seg_on[3] = obstacle_grid[col_index * 3 + 2]; // D

        // 车子也画在底层 D 段。如果当前扫描列等于 car_col，
        // 且顶层允许车子可见，就点亮 D 段形成闪烁车身。
        if (car_visible && (car_col == col_index)) begin
            seg_on[3] = 1'b1;
        end

        // 默认不选中任何位，再把当前位拉低选中。
        dig = 6'b111111;
        dig[digit_index] = 1'b0;
