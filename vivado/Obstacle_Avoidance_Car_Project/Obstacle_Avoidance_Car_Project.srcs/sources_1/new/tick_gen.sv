...
// 通用 tick enable 发生器。
//
// 本项目坚持单时钟域设计：所有模块都使用同一个 clk，不从逻辑中再分出
// 慢时钟。tick_gen 的输出 tick 只是一个 clk 周期宽度的“使能脉冲”，下游
// 模块在 if (tick) 条件内执行低速动作。
//
// 例子：
// - CLK_HZ=50_000_000, TICK_HZ=2    -> 每 25_000_000 拍拉高一次 tick。
// - CLK_HZ=50_000_000, TICK_HZ=6000 -> 用于数码管快速扫描。
// -----------------------------------------------------------------------------
module tick_gen #(
    // 输入时钟频率。
    parameter int CLK_HZ  = 50_000_000,
    // 目标 tick 频率。tick 每次只持续 1 个 clk 周期。
    ...
    // 若 tick_hz <= 0 或分频结果小于 1，则退化为每拍都出 tick。
    ...
    // 计到 DIVIDER-1 时输出 tick，因此完整周期正好是 DIVIDER 个 clk。
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            // 分频周期结束：计数器回零，同时 tick 拉高一拍。
            cnt  <= '0;
            tick <= 1'b1;
        end else begin
            // 分频周期进行中：继续计数，tick 保持低电平。
            cnt  <= cnt + 1'b1;
            tick <= 1'b0;
        end
    end
